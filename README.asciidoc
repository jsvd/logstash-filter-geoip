...
toc::[]
:logstash-ref: https://www.elastic.co/guide/en/logstash/current/plugins-outputs-elasticsearch.html
endif::[]
...
| <<plugins-{type}s-{plugin}-database>> |<<path,a valid filesystem path>>|No
...
For common options for {type}s please check the https://github.com/elastic/logstash/blob/master/docs/include/{type}.asciidoc#common-options[logstash documentation].

===== Value types of configuration settings
[[array]][[boolean]][[bytes]][[codec]][[hash]][[number]][[password]][[uri]][[path]][[string]]

For information about value types in configuration options check https://www.elastic.co/guide/en/logstash/current/configuration-file-structure.html#plugin-value-types[logstash documentation].
